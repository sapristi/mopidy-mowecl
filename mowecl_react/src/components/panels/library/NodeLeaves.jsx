...
const getButtons = (node) => {
  return match(node)
    ...
    .on(
      (node) => node.uri.startsWith("bookmark:"),
      () => <BMButtons node={node} />,
    )
    .on(
...
const NodeText = ({ node, rootElem, dispatch, mopidy }) => {
  if (isLeaf(node)) {
    ...
    return (
      <Button
        onClick={() => toggleNode(node, dispatch, mopidy)}
        style={{
          padding: 0,
          width: "100%",
          justifyContent: "left",
      ...
      >
        <Typography
          style={rootElem ? { fontWeight: 500 } : { wordBreak: "break-word" }}
        >
      ...
      <div>
        {node.expanded && node.children && (
          <div style={{ display: "flex", flexDirection: "row" }}>
            <ChildrenSideBar
              callback={() => toggleNode(node, dispatch, mopidy)}
