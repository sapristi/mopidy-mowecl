...
import { Card, CardMedia, Paper, Typography, CardContent, ButtonGroup } from '@mui/material';
...
const TrackItem = ({name, uri}) => {
    ...
    return (
        <Card sx={{
            height: 30, display: "flex", flexDirection: "row", justifyContent: "space-between",
        }}>
...
export const ExplorePanel = () => {
    ...
    for (let item of items) {
        ...
        else if (item.type == "track") {tracks.push(item)}
        else {console.warn("Unhandled type", item)}
    }
    ...
    return (
        <Paper sx={{padding: "30px"}}>
          ...
          <div style={{display: "flex", flexWrap: "wrap", gap: 10}}>
            {
                albums.map(album => <AlbumItem name={album.name} uri={album.uri} key={album.uri}/>)
            }
          </div>
          <Typography variant="h4">Top tracks</Typography>
          <div style={{display: "flex", gap: 5, flexDirection: "column"}}>
            {
                tracks.map(track => <TrackItem name={track.name} uri={track.uri} key={track.uri}/>)
            }
          </div>
        </Paper>
    )
