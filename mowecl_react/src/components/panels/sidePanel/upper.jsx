...
const MopidyStatus = () => {
  ...
  const pendingRequestsNb = useSelector(
    (state) =>
      state.mopidy.pendingRequestsNb + state.bookmarks.pendingRequestsNb,
  );
  ...
  return (
    <div>
      <CircularProgress size={30} thickness={5} style={style} {...props} />
    </div>
  );
};
...
const refreshAll = async function (mopidyCli, dispatch) {
  ...
  mopidyCli.playlists.asList().then((playlists) =>
    dispatch({
