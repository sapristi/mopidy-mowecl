...
const PlaybackUpdaterUnc = ({
  ...
  useEffect(() => {
    if (
      ...
      setTimeout(
        () => dispatch({ type: "INCR_PLAYBACK" }),
        parseInt(seek_update_interval),
      );
    }
  });
  return null;
...
export const PlaybackSlider = ({ track_length }) => {
  ...
  const dispatch = useDispatch();
  const time_position = useSelector(
    (state) => state.playback_state.time_position,
  );
  ...
  const handleChangeCommitted = (event, newValue) => {
    mopidy.playback.seek({
      time_position: Math.ceil((newValue * track_length) / sliderSteps),
    });
  };
  ...
  if (track_length) {
    return (
      ...
      >
        <PlaybackUpdater />
        <div style={{ width: "40px" }}>{duration_to_human(time_position)}</div>
        <Slider
          value={sliderValue}
          ...
          valueLabelFormat={(args) =>
            duration_to_human((args * track_length) / sliderSteps)
          }
          style={{ flex: 1, marginRight: "10px", marginLeft: "10px" }}
          color="primary"
          size="small"
        />
        <div>{duration_to_human(track_length)}</div>
      </div>
    );
  } else {
    return (
      ...
      >
        <PlaybackUpdater />
        <div>{duration_to_human(time_position)}</div>
        <LinearProgress
