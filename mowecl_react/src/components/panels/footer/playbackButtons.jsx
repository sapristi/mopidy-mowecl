...
export const PlaybackButtons = memo(({ playbackState }) => {
  ...
  const small_screen = useSelector(
    (state) => state.settings.persistant.generic.small_screen,
  );
...
export const TracklistStateButtons = memo(() => {
  const mopidyCli = useSelector((state) => state.mopidy.client);
  const tracklistState = useSelector(
    (state) => state.playback_state.tracklist_state,
  );
  ...
  return (
    <ButtonGroup orientation="vertical" size="small">
      {Object.entries(tracklistState).map(([name, value]) => (
        <Button
          onClick={makeAction(name, value)}
