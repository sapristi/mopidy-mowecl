  ...
  window.static_settings_enabled === "true"
    ? {
        ...
        generic: {
          ...
          small_screen: parseBoolean(
            window.static_settings.generic.small_screen,
          ),
        },
      }
...
export const settingsSchema = {
  ...
  generic: {
    ...
    seek_update_interval: {
      ...
      validate: (v) =>
        parseInt(v) || staticSettings.generic.seek_update_interval,
    },
    search_history_length: {
      ...
      validate: (v) =>
        nonNull([parseInt(v), staticSettings.generic.search_history_length]),
    },
    disable_dnd: {
      type: "param",
      inputType: "select",
      ...
      validate: (v) =>
        nonNull([parseBoolean(v), staticSettings.generic.disable_dnd]),
    },
    small_screen: {
      type: "param",
      inputType: "select",
      ...
      validate: (v) =>
        nonNull([parseBoolean(v), staticSettings.generic.small_screen]),
    },
  },
  colors: {
    name: "Theme colors",
    ...
    background: {
      ...
      validate: (v) =>
        validate_hex_color(v) ? v : staticSettings.colors.background,
    },
    text: {
      type: "param",
      name: "Text",
    ...
    primary: {
      ...
      validate: (v) =>
        validate_hex_color(v) ? v : staticSettings.colors.primary,
    },
  },
  globalKeys: {
    type: "group",
    ...
    description: (
      <Typography>
        Leave a field blank to deactivate. Click the "Input keys" button to
  ...
  ObjectComp(
    ...
    ([k, v]) =>
      match(schema[k].type)
        ...
        .on("param", () => [k, v.validate(settings[k])])
        .otherwise(null),
    (x) => x,
  );
...
export const settingsReducer = (state = defaultSettings, action) =>
  match(action.type)
    ...
    .on("SET_THEME", () => ({ ...state, theme: action.data }))
    .on("URI_SCHEMES", () => ({ ...state, uri_schemes: action.data }))
    .otherwise(() => state);
