    ...
    if (target.length === 1) {
      ...
      if (Array.isArray(new_children))
        new_children = new_children.map((child) => {
          return { ...child, path: [...path, child.uri] };
        });
...
export const updateChildren = (node, target, new_children) => {
  return setChildren(node, target, (previous_children) => {
    if (!previous_children) return new_children;
    return new_children.map((new_child) => {
      const previous_child = previous_children.find(
        (c) => c.uri === new_child.uri,
      );
      if (previous_child) {
        return { ...new_child, children: previous_child.children };
      } else {
        return new_child;
      }
    });
...
export const updateOrInsert = (prevItems, newItem) => {
  ...
  if (prevUris.includes(newItem.uri)) {
    return prevItems.map((item) => (item.uri === newItem.uri ? newItem : item));
  } else {
